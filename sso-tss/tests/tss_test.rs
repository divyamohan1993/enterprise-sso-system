use sso_common::types::{Receipt, TokenClaims};
...
fn token_built_and_verifiable() {
    ...
    let msg = [sso_common::domain::FROST_TOKEN, claims_bytes.as_slice()].concat();
...
fn token_claims_preserved() {
    ...
    let deserialized: sso_common::types::Token = postcard::from_bytes(&serialized).unwrap();
