...
/// Serde helper for `[u8; 64]` since serde only natively supports arrays up to 32.
mod byte_array_64 {
...
pub struct SigningRequest {
    ...
    /// Key used to verify receipt HMAC signatures. In production this would be
    /// looked up per-step from an HSM-backed key store.
