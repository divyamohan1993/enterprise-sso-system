...
use sso_common::types::{ModuleId, Receipt, Token};
use sso_crypto::entropy::generate_key_64;
use sso_crypto::receipts::sign_receipt;
use sso_opaque::messages::{OpaqueRequest, OpaqueResponse};
use sso_orchestrator::ceremony::{CeremonySession, CeremonyState, CEREMONY_TIMEOUT_SECS};
use sso_orchestrator::messages::{OrchestratorRequest, OrchestratorResponse};
use sso_orchestrator::service::OrchestratorService;
use sso_shard::transport::{connect, ShardListener};
...
async fn orchestrator_processes_auth() {
    ...
    let opaque_handle = tokio::spawn(async move {
        ...
        let mut receipt = Receipt {
            ...
            timestamp: now,
            nonce: sso_crypto::entropy::generate_nonce(),
            signature: vec![],
            ttl_seconds: 30,
        };
