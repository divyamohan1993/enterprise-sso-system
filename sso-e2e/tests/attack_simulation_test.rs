...
use sso_crypto::ct::ct_eq_64;
use sso_crypto::entropy::generate_nonce;
use sso_crypto::receipts::{hash_receipt, sign_receipt, verify_receipt_signature, ReceiptChain};
use sso_crypto::threshold::{dkg, threshold_sign};
...
fn test_attack_token_replay_across_sessions() {
    ...
    let mut signers: Vec<_> = dkg_result.shares.into_iter().take(3).collect();
    let token = build_token(&claims_session1, &mut signers, &dkg_result.group)
...
fn test_attack_domain_separation_prevents_cross_protocol_reuse() {
    ...
    mac_a.update(sso_common::domain::RECEIPT_SIGN);
    mac_a.update(data);
    ...
    let mut mac_b =
        HmacSha256::new_from_slice(&key).expect("HMAC key valid");
    mac_b.update(sso_common::domain::TOKEN_TAG);
    mac_b.update(data);
    ...
    let mut mac_c =
        HmacSha256::new_from_slice(&key).expect("HMAC key valid");
    mac_c.update(sso_common::domain::SHARD_AUTH);
    mac_c.update(data);
