...
use sso_common::types::{Token, TokenClaims, TokenHeader};
use sso_crypto::threshold;
...
fn valid_token_verifies() {
    ...
    let result = sso_verifier::verify_token(&token, &dkg.group.public_key_package);
...
fn wrong_group_key_rejected() {
    ...
    let result = sso_verifier::verify_token(&token, &dkg2.group.public_key_package);
    assert!(result.is_err());
    assert!(
        matches!(result.unwrap_err(), MilnetError::CryptoVerification(_)),
        "expected CryptoVerification error"
