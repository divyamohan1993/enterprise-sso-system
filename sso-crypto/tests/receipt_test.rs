...
fn make_receipt(step: u8, prev_hash: [u8; 32]) -> Receipt {
    Receipt {
...
fn mismatched_session_id_rejected() {
    let mut r1 = make_receipt(1, [0u8; 32]);
...
fn receipt_signature_roundtrip() {
    ...
    let mut receipt = make_receipt(1, [0u8; 32]);
