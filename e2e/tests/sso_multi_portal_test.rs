...
impl ServicePortal {
    fn new(name: &str, required_tier: u8, required_scope: u32, key: &PublicKeyPackage) -> Self {
        Self {
            name: name.to_string(),
            required_tier,
            required_scope,
    ...
    fn check_access(&self, token: &Token) -> Result<String, String> {
        ...
        let claims = verify_token(token, &self.verifying_key)
...
async fn test_sso_single_login_multiple_portals() {
    ...
    let portals = vec![
        ServicePortal::new("Command Dashboard", 2, 0x01, &group_key),
        ServicePortal::new("Personnel Records", 2, 0x02, &group_key),
        ServicePortal::new("Communications", 2, 0x04, &group_key),
        ServicePortal::new("Logistics", 2, 0x08, &group_key),
        ServicePortal::new("Intelligence Reports", 2, 0x03, &group_key),
    ];
...
async fn test_sso_token_works_across_independent_verifiers() {
    ...
    let token = build_signed_token(&claims, &mut signers, &dkg_result.group);
...
async fn test_sso_different_users_different_tokens_same_portals() {
    ...
    let mut store = CredentialStore::new();
    store.register("alice", b"alice_pass");
    store.register("bob", b"bob_pass");
    let (gateway_addr, group_key) = boot_full_system(store).await;
    ...
    let portal = ServicePortal::new("Shared Portal", 2, 0x01, &group_key);
...
async fn test_sso_scope_restricts_portal_access() {
    ...
    let portal_01 = ServicePortal::new("Scope 0x01", 2, 0x01, &group_key);
    let portal_02 = ServicePortal::new("Scope 0x02", 2, 0x02, &group_key);
    ...
    let portal_04 = ServicePortal::new("Scope 0x04", 2, 0x04, &group_key);
...
async fn test_attack_forged_scope_escalation() {
    ...
    let portal = ServicePortal::new("Restricted", 2, 0xFF, &group_key);
...
async fn test_attack_forged_tier_escalation() {
    ...
    let portal = ServicePortal::new("Sovereign", 1, 0x01, &group_key);
    let result = portal.check_access(&tampered);
    assert!(
        result.is_err(),
...
async fn test_attack_expired_token_at_portal() {
    ...
    let portal = ServicePortal::new("Portal", 2, 0x01, &group_key);
    let result = portal.check_access(&token);
...
async fn test_attack_token_from_rogue_sso_server() {
    ...
    let rogue_dkg = dkg(5, 3);
    let mut rogue_signers: Vec<_> = rogue_dkg.shares.into_iter().take(3).collect();

    // Sign a token with the rogue server's key
    let claims = make_claims(Uuid::new_v4(), 2, 0xFF, 600);
    let rogue_token = build_signed_token(&claims, &mut rogue_signers, &rogue_dkg.group);

    // Portal trusts the REAL SSO server's key
    let portal = ServicePortal::new("Trusted Portal", 2, 0x01, &real_key);
...
async fn test_sso_concurrent_portal_access() {
    ...
    // 3 portals
    let portals = vec![
        ServicePortal::new("Portal Alpha", 2, 0x01, &group_key),
        ServicePortal::new("Portal Beta", 2, 0x02, &group_key),
        ServicePortal::new("Portal Gamma", 2, 0x04, &group_key),
    ];
