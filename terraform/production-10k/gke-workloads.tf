...
#
# SECURITY NOTE: Network policies are IDENTICAL to production-1k.
# Security does not change with scale — the quantum-safe architecture
# (SHARD protocol, FROST 3-of-5, ML-DSA-87, X-Wing KEM, BFT audit)
# is the same at 1K and 10K. Only quotas scale.
###############################################################################
...
#
# UNCHANGED FROM 1K: Same policies, same ports, same zero-trust matrix.
###############################################################################
...
###############################################################################
# Resource Quotas — Scaled for 10K req/s
# CHANGED: 3x CPU, 3x memory, 3x pods vs 1K deployment
# These are hard caps to prevent runaway scaling — HPA operates within these
###############################################################################
...
resource "kubernetes_resource_quota" "milnet_sso" {
  ...
  spec {
    hard = {
      "requests.cpu"           = "240"   # 3x the 1K quota (80 -> 240)
      "requests.memory"        = "480Gi" # 3x the 1K quota (160 -> 480)
      "limits.cpu"             = "360"   # 3x the 1K quota (120 -> 360)
      "limits.memory"          = "768Gi" # 3x the 1K quota (256 -> 768)
      "pods"                   = "600"   # 3x the 1K quota (200 -> 600)
      "services"               = "50"    # Slightly more services
      "secrets"                = "100"   # 2x for more replicas needing secrets
      "configmaps"             = "100"   # 2x for more replica configs
      "persistentvolumeclaims" = "30"    # More PVCs for audit storage
    }
  }
}
...
# CHANGED: Higher max per container for compute-heavy pods at 10K scale
###############################################################################
...
resource "kubernetes_limit_range" "milnet_sso" {
  ...
  spec {
    limit {
      ...
      default = {
        cpu    = "1000m" # Doubled default: 10K pods need more CPU
        memory = "1Gi"   # Doubled default: more connections, bigger buffers
      }

      default_request = {
        cpu    = "250m"  # Higher base request for 10K scheduling accuracy
        memory = "256Mi" # Doubled request
      }

      max = {
        cpu    = "8"     # Doubled: OPAQUE pods may need up to 8 cores
        memory = "16Gi"  # Doubled: larger connection pools at 10K
      }
