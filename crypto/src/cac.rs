...
///
/// Returns the raw SPKI bytes (from tag to end of value).
fn extract_spki_bytes(cert_der: &[u8]) -> Option<Vec<u8>> {
    ...
    // Walk TBSCertificate to the subjectPublicKeyInfo field (7th field, index 6).
    let mut pos = 0;
    let mut field_index = 0usize;
    ...
    while pos < tbs.len() {
        ...
        if field_index == 5 {
            ...
            if tag == 0x30 {
                // Return the raw SPKI including its tag and length.
                let spki_start = pos - (next - pos + /* rewind */ 0);
                // Reconstruct the full TLV from tbs.
                // The `value` slice points inside tbs; compute its offset.
                let value_offset = value.as_ptr() as usize - tbs.as_ptr() as usize;
                // TLV starts before value by header_len bytes.
                let header_len = pos - value_offset - value.len();
                let tlv_start = value_offset - header_len;
                let _ = spki_start; // suppress warning
