...
use sso_audit::log::{hash_entry, AuditLog};
use sso_common::actions::{
    check_action_authorization, validate_multi_person_ceremony, ActionToken, CeremonyParticipant,
};
use sso_common::network::is_permitted_channel;
...
use sso_crypto::receipts::{hash_receipt, sign_receipt};
use sso_crypto::threshold::{dkg, threshold_sign, verify_group_signature};
use sso_crypto::xwing::{xwing_decapsulate, xwing_encapsulate, XWingKeyPair};
use sso_gateway::puzzle::{solve_challenge, PuzzleChallenge, PuzzleSolution};
use sso_gateway::server::{GatewayServer, OrchestratorConfig};
use sso_gateway::wire::{AuthRequest, AuthResponse};
use sso_kt::merkle::MerkleTree;
use sso_opaque::messages::OpaqueRequest;
use sso_opaque::service::handle_request as opaque_handle_request;
use sso_opaque::store::CredentialStore;
use sso_orchestrator::service::OrchestratorService;
use sso_ratchet::chain::RatchetChain;
use sso_risk::scoring::{RiskEngine, RiskLevel, RiskSignals};
use sso_risk::tiers::check_tier_access;
use sso_shard::protocol::ShardProtocol;
use sso_shard::transport::ShardListener;
use sso_tss::messages::{SigningRequest, SigningResponse};
use sso_tss::token_builder::build_token;
use sso_tss::validator::validate_receipt_chain;
use sso_verifier::verify::verify_token;
use uuid::Uuid;
...
async fn boot_opaque(store: CredentialStore) -> String {
    ...
    tokio::spawn(async move {
        loop {
            ...
            let response_bytes = postcard::to_allocvec(&response).expect("serialize OPAQUE response");
...
async fn boot_tss(
    mut signers: Vec<sso_crypto::threshold::SignerShare>,
    group: sso_crypto::threshold::ThresholdGroup,
) -> String {
...
async fn boot_orchestrator(opaque_addr: String, tss_addr: String) -> String {
    ...
    tokio::spawn(async move {
        loop {
            ...
            let request: sso_orchestrator::messages::OrchestratorRequest =
...
async fn test_complete_tier2_auth_flow() {
    ...
    let claims = verify_token(&token, &group_key).expect("token verification should succeed");
...
async fn test_multiple_users_concurrent_auth() {
    ...
    for i in 0..5 {
        let uid = store.register(&format!("user{i}"), format!("pass{i}").as_bytes());
        user_ids.push(uid);
    }
    let (gateway_addr, group_key) = boot_full_system(store).await;
    ...
    for i in 0..5 {
        ...
        handles.push(tokio::spawn(async move {
            ...
            let token_bytes = resp.token.unwrap();
            let token: Token = postcard::from_bytes(&token_bytes).expect("deserialize token");
            let claims = verify_token(&token, &gk).expect("token should verify");
...
async fn test_sequential_auth_sessions() {
    ...
    for _ in 0..5 {
        ...
        let token_bytes = resp.token.unwrap();
        let token: Token = postcard::from_bytes(&token_bytes).expect("deserialize token");
        let claims = verify_token(&token, &group_key).expect("token should verify");
...
async fn test_unicode_username_works() {
    ...
    store.register(username, b"unicodepass");
    let (gateway_addr, group_key) = boot_full_system(store).await;
...
async fn test_very_long_password_works() {
    ...
    store.register("longpass_user", &long_pass);
    let (gateway_addr, group_key) = boot_full_system(store).await;
...
fn test_token_signature_cannot_be_transplanted() {
    ...
    let result = verify_token(&franken_token, &group_key);
...
async fn test_shard_100_messages_sequential() {
    ...
    for i in 0u32..100 {
        let mut transport =
            sso_shard::transport::connect(&addr, ModuleId::Gateway, SHARD_HMAC_KEY)
                .await
                .expect("connect");
...
async fn test_shard_replay_detected_even_under_load() {
    ...
    let mut transport = sso_shard::transport::ShardTransport::new(stream, protocol);
