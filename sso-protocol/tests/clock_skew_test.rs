...
#[test] fn token_exp_iat() { big(|| { let t = create_id_token("https://c", &uuid::Uuid::new_v4(), "c", None, &KEY); let c = verify_id_token(&t, KEY.verifying_key()).unwrap(); assert!(c.exp > c.iat); assert_eq!(c.exp - c.iat, 3600); }); }
...
#[test] fn ratchet_epochs() { use ratchet::chain::RatchetChain; let mut c = RatchetChain::new(&[0x11u8;64]); assert_eq!(c.epoch(), 0); for i in 1..=10u64 { c.advance(&[0xAA;32], &[0xBB;32]); assert_eq!(c.epoch(), i); } assert!(!c.is_expired()); }
